...
while(True):
    ...
    if(iterations >= 6):
        for key, value in point_counts.items():
            if value >=3:
                print "colors is", point_colors[key]
                serial.write(point_colors[key])
                break
        iterations = 0
        point_counts = {}
        point_colors = {}
        ...
        if(num_rectangles < curr_num_rectangles):
            for idx, point in enumerate(curr_rectangle_points):
                ...
                if(addedBlock):
                    ...
                    if(tuple(point[0][0]) not in point_counts.keys()):
                        point_counts[tuple(point[0][0])] = 0
                        point_colors[tuple(point[0][0])] = newColor
                    else:
                        point_counts[tuple(point[0][0])] = point_counts[tuple(point[0][0])] +1
